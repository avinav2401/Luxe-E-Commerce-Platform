...
export async function POST(req: Request) {
    try {
        ...
        if (Number(price) <= 0 || Number(stock) < 0 || Number(discount) < 0 || Number(discount) > 90) {
            return NextResponse.json({ error: 'Price, stock must be non-negative. Discount must be between 0 and 90.' }, { status: 400 });
        }
